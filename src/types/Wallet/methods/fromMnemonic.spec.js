...
describe('Wallet - fromMnemonic', function suite() {
  ...
  it('should set wallet from mnemonic', () => {
    ...
    expect(new Xazabcore.HDPrivateKey(self1.HDPrivateKey)).to.equal(self1.HDPrivateKey);
    ...
    expect(new Xazabcore.HDPrivateKey(self2.HDPrivateKey)).to.equal(self2.HDPrivateKey);
...
describe('Wallet - fromMnemonic - with passphrase', function suite() {
  ...
  it('should correctly works with passphrase', () => {
    ...
    expect(new Xazabcore.Address(pubKey1).toString()).to.equal('Xq3zjky18WjwAHpLgGLasvX5g8TeLRKaxt');
    ...
    expect(new Xazabcore.Address(pubKey2, 'testnet').toString()).to.equal('yWYCH9XDRnpdNxh67jQJFkovToBVwWr8Ck');
  });
});
